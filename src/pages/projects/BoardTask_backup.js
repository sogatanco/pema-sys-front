            ...
            <div className="board-body task">
              ...
              {addSubtaskOpen === td.task_id && (
                <TaskForm
                  ...
                  title="Create task level 2"
                  taskId={td.task_id}
                />
              )}
            </div>
            {td.subtasks.length >= 1 &&
              td.subtasks.map((st, idx) => (
                ...
                <>
                  <div key={st.task_id} className="board-body subtask">
                      ...
                      <div className="task-info">
                        ...
                        {isMemberActive && (
                          <button
                            type="button"
                            className="add-child"
                            onClick={() => setAddSubtaskOpen(st.task_id)}
                          >
                            <MaterialIcon icon="add_circle" />
                          </button>
                        )}
                    ...
                    {addSubtaskOpen === st.task_id && (
                      <TaskForm
                        {...{ projectId, setAddSubtaskOpen, refetch, type }}
                        title="Create task level 3"
                        taskId={st.task_id}
                      />
                    )}
                  </div>
                  {idx === 1 && (
                    <div key={st.task_id} className="board-body subtask-level3">
                      <div className="task-title text-muted">Subtask level 3</div>
                      <div className="task-bottom">
                        <div className="task-info">
                          {/* <small className="text-muted">{td.subtasks.length} Subtask</small> */}
                        </div>
