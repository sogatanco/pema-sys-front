...
import './TaskReviewAdditional.scss';
// hooks
...
import TaskItem from './TaskItemAdditional';
import PopupConfirmation from '../../../../components/popup/PopupConfirmation';

const TaskReviewAdditional = ({ task, title, refetch }) => {
  const {
    handleCheckItem,
    // handleCheckForAll,
    isAllChecked,
          ...
          <tbody>
            <>
              <tr className="task-parent">
                ...
                <td>{/* <Progress color="blue" progress={task?.task_progress} /> */}</td>
                <td>
              ...
              <tr
                className={`task-column ${isAllChecked || selectedTasks.length ? 'bg-white' : ''}`}
              >
                <td width="30">
...
TaskReviewAdditional.propTypes = {
...
export default TaskReviewAdditional;
